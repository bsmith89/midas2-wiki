...
Metagenomic Intra-Species Diversity Analysis ([MIDAS](https://genome.cshlp.org/content/26/11/1612)) is an integrated pipeline for profiling strain-level genomic variation and gene copy number variations for metagenomics data. Its analysis steps are run against a database of 5,926 bacterial species extracted from 30,000 genomes (MIDAS DB v1.2).
...
MIDAS 2.0 contains two strain-level metagenomics analysis modules: population SNVs analysis (SNPs module) and pan-genome CNVs analysis (Genes module).  Each module includes two sequential steps: single-sample analysis and across-samples analysis. Given a collection of shotgun metagenomics samples, single-sample analysis started with building a sample-specific genome database of abundant species in the sample determined via profiling 15 universal single copy marker genes (Species module).
...
The goal of the single-sample Species module is to quickly detect the presence of species in the sample, which would be used to construct the sample-specific representative genome database (rep-genome) and pan-genome database for the SNPs and Genes module. Metagenomics sequencing reads were mapped to 15 universal single copy genes, and uniquely mapped read counts per marker gene were computed. And ambiguous mapped reads were probabilistically assigned. Only species with more than two marker genes with more than two reads are reported. Due to the simplicity of the 15 marker genes, we recommend against using the Species module for taxonomic profiling. 
